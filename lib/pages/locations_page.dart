...
class _LocationsPageState extends State<LocationsPage>{ bool loading=true; List<Map<String,dynamic>> items=[];
  ...
  Future<void> load()async{final r=await http.get(Uri.parse('${widget.baseUrl}/api/locations/'),headers:Session.authHeaders);if(mounted)setState((){items=r.statusCode==200?(jsonDecode(r.body)as List).cast<Map<String,dynamic>>():[];loading=false;});}
  Future<void> add()async{final name=TextEditingController(),address=TextEditingController();LatLng? point;
    ...
    if(saved==true&&point!=null){await http.post(Uri.parse('${widget.baseUrl}/api/locations/'),headers:Session.authHeaders,body:jsonEncode({'name':name.text,'address_text':address.text,'latitude':point!.latitude,'longitude':point!.longitude}));load();}}

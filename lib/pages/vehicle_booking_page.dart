...
class _VehicleBookingPageState extends State<VehicleBookingPage>{
  bool loading=true, slotsLoading=false; List<Map<String,dynamic>> locations=[],cars=[],services=[],addOns=[],categories=[]; Set<String> booked={},unavailable={};
  ...
  @override void initState(){super.initState();final now=DateTime.now();dates=List.generate(4,(i)=>DateTime(now.year,now.month,now.day).add(Duration(days:i)));date=dates.first;clockTimer=Timer.periodic(const Duration(minutes:1),(_){if(mounted)setState((){if(time!=null&&isBookingSlotPast(date,time!))time=null;});});load();}
  ...
  @override Widget build(BuildContext context)=>Directionality(textDirection:TextDirection.rtl,child:Scaffold(appBar:AppBar(title:const Text('غسيل السيارات')),body:loading?const Center(child:CircularProgressIndicator()):ListView(padding:const EdgeInsets.all(16),children:[
    ...
    _title('الأوقات المتاحة'),if(slotsLoading)const Center(child:CircularProgressIndicator())else Builder(builder:(context){final visible=bookingTimeSlots.where((s)=>!unavailable.contains(s)&&!isBookingSlotPast(date,s)).toList();if(visible.isEmpty)return const Text('لا توجد أوقات متاحة لهذا اليوم',style:TextStyle(color:AppColors.muted));return Wrap(spacing:8,runSpacing:8,children:visible.map((s)=>ChoiceChip(label:Text(s),selected:time==s,onSelected:booked.contains(s)?null:(_)=>setState(()=>time=s),disabledColor:Colors.grey.shade200)).toList());}),

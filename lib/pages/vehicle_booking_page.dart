...
class _VehicleBookingPageState extends State<VehicleBookingPage>{
  bool loading=true, slotsLoading=false; List<Map<String,dynamic>> locations=[],cars=[],services=[],addOns=[],categories=[],timeSlots=[]; Set<String> booked={},unavailable={};
  ...
  @override void initState(){super.initState();final now=DateTime.now();dates=List.generate(4,(i)=>DateTime(now.year,now.month,now.day).add(Duration(days:i)));date=dates.first;clockTimer=Timer.periodic(const Duration(minutes:1),(_){if(mounted)setState((){final slot=_slotForLabel(time);if(slot!=null&&isBookingSlotPast(date,slot))time=null;});});load();}
  ...
  Map<String,dynamic>? _slotForLabel(String? label){if(label==null)return null;for(final slot in timeSlots){if(slot['label']==label)return slot;}return null;}
  Future<void> load()async{final rs=await Future.wait([
    ...
    http.get(Uri.parse('${widget.baseUrl}/api/booking-time-slots/'))]);
    if(rs[0].statusCode==401||rs[1].statusCode==401){await Session.handleUnauthorized();return;}
    if(mounted)setState((){locations=rs[0].statusCode==200?(jsonDecode(rs[0].body)as List).cast<Map<String,dynamic>>():[];cars=rs[1].statusCode==200?(jsonDecode(rs[1].body)as List).cast<Map<String,dynamic>>():[];services=rs[2].statusCode==200?(jsonDecode(rs[2].body)as List).cast<Map<String,dynamic>>():[];addOns=rs[3].statusCode==200?(jsonDecode(rs[3].body)as List).cast<Map<String,dynamic>>():[];categories=rs[4].statusCode==200?(jsonDecode(rs[4].body)as List).cast<Map<String,dynamic>>():[];timeSlots=rs[5].statusCode==200?(jsonDecode(rs[5].body)as List).cast<Map<String,dynamic>>():[];loading=false;});await loadSlots();}
  Future<void> loadSlots()async{setState((){slotsLoading=true;time=null;});final r=await http.get(Uri.parse('${widget.baseUrl}/api/booked-slots/?date=${iso(date)}'));if(mounted)setState((){final data=r.statusCode==200?jsonDecode(r.body):<String,dynamic>{};booked=Set<String>.from(data['booked']??[]);unavailable=Set<String>.from(data['unavailable']??[]);slotsLoading=false;});}
  ...
  @override Widget build(BuildContext context)=>Directionality(textDirection:TextDirection.rtl,child:Scaffold(appBar:AppBar(title:const Text('غسيل السيارات')),body:loading?const Center(child:CircularProgressIndicator()):ListView(padding:const EdgeInsets.all(16),children:[
    ...
    _title('خدمات إضافية (اختياري)'),...addOns.map((x){final id=x['id'] as int,qty=addOnQuantities[id]??0,allows=x['allows_quantity']==true;return Card(child:Padding(padding:const EdgeInsets.symmetric(horizontal:8,vertical:4),child:Row(children:[Checkbox(value:qty>0,onChanged:(v)=>setState(()=>addOnQuantities[id]=v==true?1:0)),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(x['name'],style:const TextStyle(fontWeight:FontWeight.bold)),Text('${x['price']} ر.س / ${x['unit']}',style:const TextStyle(color:AppColors.muted))])),if(qty>0&&allows)...[IconButton(onPressed:qty<=1?null:()=>setState(()=>addOnQuantities[id]=qty-1),icon:const Icon(Icons.remove_circle_outline)),Text('$qty',style:const TextStyle(fontWeight:FontWeight.bold)),IconButton(onPressed:()=>setState(()=>addOnQuantities[id]=qty+1),icon:const Icon(Icons.add_circle_outline))]])));}),
    _title('الأوقات المتاحة'),if(slotsLoading)const Center(child:CircularProgressIndicator())else Builder(builder:(context){final visible=timeSlots.where((slot){final label=slot['label']?.toString()??'';return !unavailable.contains(label)&&!isBookingSlotPast(date,slot);}).toList();if(visible.isEmpty)return const Text('لا توجد أوقات متاحة لهذا اليوم',style:TextStyle(color:AppColors.muted));return Wrap(spacing:8,runSpacing:8,children:visible.map((slot){final label=slot['label']?.toString()??'';return ChoiceChip(label:Text(label),selected:time==label,onSelected:booked.contains(label)?null:(_)=>setState(()=>time=label),disabledColor:Colors.grey.shade200);}).toList());}),
    const SizedBox(height:24),Container(padding:const EdgeInsets.all(16),decoration:BoxDecoration(color:AppColors.surface,borderRadius:BorderRadius.circular(16),border:Border.all(color:AppColors.border)),child:Column(children:[_row('الخدمة',service?['name']??'لم تحدد'),_row('الإضافات','${selectedAddOns().length}'),_row('الإجمالي المبدئي','${total().toStringAsFixed(2)} ر.س'),const SizedBox(height:12),SizedBox(width:double.infinity,child:FilledButton(onPressed:next,child:const Text('تأكيد والانتقال للدفع')))])),const SizedBox(height:30)
  ])));

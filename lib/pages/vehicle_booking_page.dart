...
class _VehicleBookingPageState extends State<VehicleBookingPage>{
  bool loading=true, slotsLoading=false; List<Map<String,dynamic>> locations=[],cars=[],services=[]; Set<String> booked={};
  ...
  Future<void> load()async{final rs=await Future.wait([
    ...
    if(mounted)setState((){locations=rs[0].statusCode==200?(jsonDecode(rs[0].body)as List).cast<Map<String,dynamic>>():[];cars=rs[1].statusCode==200?(jsonDecode(rs[1].body)as List).cast<Map<String,dynamic>>():[];services=rs[2].statusCode==200?(jsonDecode(rs[2].body)as List).cast<Map<String,dynamic>>():[];loading=false;});await loadSlots();}
  ...
  @override Widget build(BuildContext context)=>Directionality(textDirection:TextDirection.rtl,child:Scaffold(appBar:AppBar(title:const Text('غسيل السيارات')),body:loading?const Center(child:CircularProgressIndicator()):ListView(padding:const EdgeInsets.all(16),children:[
    ...
    const SizedBox(height:24),Container(padding:const EdgeInsets.all(16),decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(16)),child:Column(children:[_row('الخدمة',service?['name']??'لم تحدد'),_row('السعر',service==null?'—':'${service!['price']} ر.س'),const SizedBox(height:12),SizedBox(width:double.infinity,child:FilledButton(onPressed:next,child:const Text('تأكيد والانتقال للدفع')))])),const SizedBox(height:30)
  ])));
